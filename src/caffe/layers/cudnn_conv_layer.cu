  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    for (int g = 0; g < this->group_; g++) {
      ...
      if (this->param_propagate_down_[0]) {
       ...
       //ICCV17 BIER
       if(this->layer_param_.convolution_param().bier_init())
       {
        const int N_ = bier_temp_.count();//num_output;
        const int K_ = this->blobs_[0]->count()/N_;//
        Dtype lamda_ = this->layer_param_.convolution_param().lamda();
