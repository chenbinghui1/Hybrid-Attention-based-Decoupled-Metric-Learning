...
void OrderLossLayer<Dtype>::Forward_cpu(
    ...
    for(int i=0; i < class_number; i++)
    {
        ...
        for(int j=0; j<per_number/2; j++)
        {
                caffe_cpu_axpby(channels, Dtype(alpha1*2.0/per_number), temp1_.cpu_data(), Dtype(1), bottom[0]->mutable_cpu_diff() + (i * per_number + j) * channels);
        }
        for(int j=per_number/2; j<per_number; j++)
        {
                caffe_cpu_axpby(channels, Dtype(-2.0*alpha1/per_number), temp1_.cpu_data(), Dtype(1), bottom[0]->mutable_cpu_diff() + (i * per_number + j) * channels);
        }
        //second order
