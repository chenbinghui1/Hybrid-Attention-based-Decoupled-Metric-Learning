...
#include "caffe/layers/multi_proposal_crop_layer.hpp"
...
void MultiProposalCropLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top) {
}

template<typename Dtype>
void MultiProposalCropLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top) {
        CHECK_EQ(bottom[0]->shape(1),3)<<"The Input Images Are Not RGB Images";
        int out_size = this->layer_param_.multi_proposal_crop_param().out_size();
    vector<int> top_shape = bottom[0]->shape();
	top_shape[2] = out_size;
	top_shape[3] = out_size;
    top[0]->Reshape(top_shape);
    int layer_num = bottom.size()-1;
    for(int i = 1; i <= layer_num; i++){
        M_.push_back(vector<Dtype>(bottom[i]->shape(3)*bottom[i]->shape(3),Dtype(0.)));
        D_.push_back(vector<Dtype>(bottom[i]->shape(3)*bottom[i]->shape(3)*bottom[i]->shape(3)*bottom[i]->shape(3),Dtype(0)));
        tmp_.push_back(vector<Dtype>(bottom[i]->shape(3)*bottom[i]->shape(3),Dtype(0.)));
        tmp_M_.push_back(vector<Dtype>(bottom[i]->shape(3)*bottom[i]->shape(3),Dtype(0)));
    }
}

template<typename Dtype>
void MultiProposalCropLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top) {
        
        
        const Dtype mean_value1 = this->layer_param_.multi_proposal_crop_param().mean_value1();
	const Dtype mean_value2 = this->layer_param_.multi_proposal_crop_param().mean_value2();
	const Dtype mean_value3 = this->layer_param_.multi_proposal_crop_param().mean_value3();
	Dtype mean_value[3] = { mean_value1, mean_value2, mean_value3 };
	const int layer_num = bottom.size()-1;
	const int in_size = bottom[0]->shape(3);
	const int out_size = this->layer_param_.multi_proposal_crop_param().out_size();
	...
	for (int n = 0; n<bottom[0]->shape(0); n++)
	{
	  ...
	  caffe_set(bottom[1]->shape(3)*bottom[1]->shape(3), Dtype(1.0/Dtype(bottom[1]->shape(3))/Dtype(bottom[1]->shape(3))), &M_[0][0]);
	  //layer to layer 
	  for(int layer = 0; layer < layer_num; layer++){
		const int N = bottom[layer+1]->shape(3);
	        const int channels = bottom[layer+1]->channels();
	        const Dtype ems = Dtype(0.15*N);

	        
	        const Dtype* features = bottom[layer+1]->cpu_data();// feature maps
	        Dtype* tmp = &tmp_[layer][0];
	        Dtype* M = &M_[layer][0];
	        Dtype* tmp_M = &tmp_M_[layer][0];
	        Dtype* D = &D_[layer][0];
		
		//compute transforming matrix
		//caffe_set(N*N, Dtype(1.0/N/N), M);
		//update current M from the former layer's M
		if(layer > 0){
		        //convert the former M to cv::Mat
		        cv::Mat img_M = cv::Mat(bottom[layer]->shape(3), bottom[layer]->shape(3), CV_8UC1);
		        for (int i = 0; i < bottom[layer]->shape(3); i++)//h
		        {
			        uchar* ptr = img_M.ptr<uchar>(i);
			        int img_index = 0;
			        for (int j = 0; j < bottom[layer]->shape(3); j++)//w
			        {
					ptr[img_index++] = 255*M_[layer-1][i*bottom[layer]->shape(3)+j];//norm M to [0-255]
					
			        }
		        }
		        cv::Mat resize_img_M = cv::Mat(N, N, CV_8UC1);
		        //resize the former M to current size
		        cv::resize(img_M, resize_img_M, resize_img_M.size(), 0, 0, cv::INTER_LINEAR);
		        //convert CV::Mat to M
		        int top_index;
		        for (int i = 0; i < N; i++)//h
		        {
			        const uchar* ptr = resize_img_M.ptr<uchar>(i);
			        int img_index = 0;
			        for (int j = 0; j < N; j++)//w
			        {
					Dtype pixel = static_cast<Dtype>(ptr[img_index++]);
					top_index = i * N + j;
					M[top_index] = pixel / Dtype(255);
			        }
		        }
		}
		for(int j=0; j<N*N; j++)
		...
		for (int i = 0; i<N*N; i++)
		{
		        M[i] = (M[i] - min_M)/(max_M - min_M);//scale M to [0-1]
		}

	  }
		const int N = bottom[layer_num]->shape(3);
		Dtype* M = &M_[layer_num-1][0];
		//convert M to cv::Mat
		...
		for (int i = 0; i < N; i++)//h
		{
			...
			for (int j = 0; j < N; j++)//w
			{
					ptr[img_index++] = 255*M[i*N+j];//norm M to [0-255]
...
template<typename Dtype>
void MultiProposalCropLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(MultiProposalCropLayer);
#endif

INSTANTIATE_CLASS(MultiProposalCropLayer);
REGISTER_LAYER_CLASS(MultiProposalCrop);
