...
void BinomialLossLayer<Dtype>::LayerSetUp(
  ...
  for(int i = 0; i < bottom.size()-1; i++){
        center_temp_.push_back(vector<Dtype>(bottom[i]->channels(),Dtype(0.)));//
  }
}
template <typename Dtype>
void BinomialLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    const Dtype* label = bottom[bottom.size()-1]->cpu_data();
    int num = bottom[0]->num();//
    int M = bottom.size()-1;//number of learners
    Dtype beta = this->layer_param_.binomial_loss_param().beta();
    Dtype alpha = this->layer_param_.binomial_loss_param().alpha();
    Dtype cost_p = this->layer_param_.binomial_loss_param().cost_p();
    Dtype cost_n = this->layer_param_.binomial_loss_param().cost_n();
    Dtype shrinkage = this->layer_param_.binomial_loss_param().shrinkage();
    Dtype loss = 0.;
    ...
    if(this->layer_param_.binomial_loss_param().perturbation())
    {
        if(F_iter_size_ == 0)
        {
                top[0]->mutable_cpu_data()[0] = top[0]->mutable_cpu_data()[0] * (Dtype(1) - this->layer_param_.binomial_loss_param().perturbation_loss_weight());
                F_iter_size_ = 1;
        }
        else
        {
                top[0]->mutable_cpu_data()[0] = top[0]->mutable_cpu_data()[0] * this->layer_param_.binomial_loss_param().perturbation_loss_weight();
                F_iter_size_ = 0;
        }
    }
...
template <typename Dtype>
void BinomialLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) 
{
    ...
    if(this->layer_param_.binomial_loss_param().perturbation())
    {
        if(B_iter_size_ == 0)
        {
                for(int m = 0; m < bottom.size()-1; m++)
               {   
                caffe_cpu_scale(bottom[m]->count(),(Dtype(1)-this->layer_param_.binomial_loss_param().perturbation_loss_weight()),bottom[m]->cpu_diff(),bottom[m]->mutable_cpu_diff());
                }
                B_iter_size_ = 1;
        }
        else
        {
                for(int m = 0; m < bottom.size()-1; m++)
               {   
                caffe_cpu_scale(bottom[m]->count(),Dtype((this->layer_param_.binomial_loss_param().perturbation_loss_weight())),bottom[m]->cpu_diff(),bottom[m]->mutable_cpu_diff());
                }
                B_iter_size_ = 0;
        }
    }
...
#ifdef CPU_ONLY
STUB_GPU(BinomialLossLayer);
#endif

INSTANTIATE_CLASS(BinomialLossLayer);
REGISTER_LAYER_CLASS(BinomialLoss);
